...
int GetProcId(char* ProcName)
{
	...
	{
		do{
			if( strcmp((char*)pe32.szExeFile, ProcName ) == 0 )
				break;
...
void __main(bool flag)
{
		...
		cout << endl;
		int PID = GetProcId("FTK Imager.exe");	
...
void getFTKaddress()
{
	...
	char* tvaddr  = "\\AccessData\\FTK Imager\\*.dll";
	char* path = new char[strlen(appdata)+strlen(tvaddr)+1];

	sprintf(path, "%s%s", appdata, tvaddr);

	cout << "Get FTK  address\n";

	HANDLE hFind;
	WIN32_FIND_DATA data;

	hFind = FindFirstFile((wchar_t*) path, &data);
